...
@api_view(['POST'])
@permission_classes([])  # Internal API, protected by network/gateway
def generate_sso_token(request):
    """
    Generate a short-lived signed token for SSO login.
    Called by Brain API Gateway.
    """
    ...
    try:
        ...
        # In production, use SITE_URL setting
        login_url = f"/admin/sso/login/?token={token}"
