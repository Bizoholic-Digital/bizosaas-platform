...
try:
    ...
    VAULT_ENABLED = False
    print("⚠️  Vault helper not available, using environment variables")
...
# Database - Using Vault for credentials
if VAULT_ENABLED:
    db_config = get_database_config()
    DATABASES = {
        'default': {
            'ENGINE': 'django.db.backends.postgresql',
            'NAME': config('DB_NAME', default='django_crm'),  # Database name from docker-compose
...
# Cache Configuration (Redis) - Using Vault for connection string
if VAULT_ENABLED:
...
# Celery Configuration (for background tasks) - Using Vault for Redis connection
if VAULT_ENABLED:
    redis_config = get_redis_config()
