    ...
    const fetchData = async () => {
        ...
        try {
            if (activeTab === 'cms-pages') {
                ...
                const response = await fetch('/api/brain/wagtail/posts');
                if (response.ok) {
                    const data = await response.json();
                    setPosts(data.items || []);
                } else {
                ...
                const response = await fetch('/api/brain/wagtail/media');
                if (response.ok) {
                    const data = await response.json();
                    setMedia(data.items || []);
                } else {
                    console.error('Failed to fetch media, using empty array');
                    setMedia([]);
                }
            }
        } catch (error) {
            console.error('Failed to fetch CMS data:', error);
            // Set empty arrays on error to show "no items" message instead of crash
            if (activeTab === 'cms-pages') setPages([]);
            if (activeTab === 'cms-posts') setPosts([]);
            if (activeTab === 'cms-media') setMedia([]);
        } finally {
            setIsLoading(false);
        }
    };
