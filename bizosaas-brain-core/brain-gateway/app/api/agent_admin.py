...
async def list_optimizations(
    ...
    """List all AI-suggested agent optimizations (Admin View)"""
    optimizations = AgentService.get_agent_optimizations(db, agent_id)
    
    total_savings = 0.0
    for opt in optimizations:
        if opt.status == 'pending' and opt.potential_savings:
            total_savings += opt.potential_savings.get("amount", 0.0)
...
@router.post("/optimizations/{optimization_id}/toggle-auto")
