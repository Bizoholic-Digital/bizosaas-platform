...
async def get_me(
    ...
    if subscription:
        # Check if it's a local model or a PortSubscription
        if hasattr(subscription, 'plan') and subscription.plan:
            features_dict = subscription.plan.features or {}
            plan_features = features_dict.get('feature_slugs', [])
        else:
            # Look up plan features by slug/code
            plan_slug = subscription.plan_id
