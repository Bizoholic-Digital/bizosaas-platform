...
async def get_dependency_health() -> Dict[str, Any]:
    ...
    # 3. Temporal
    temporal_addr = os.getenv("TEMPORAL_ADDRESS", "").strip() or os.getenv("TEMPORAL_HOST", "").strip() or "temporal:7233"
    
    # Basic check if port is open (simplified)
    import socket
    try:
        host_part = temporal_addr
        if ":" in host_part:
        ...
        logger.info(f"Health check: Testing Temporal connection to {host}:{port}")
        with socket.create_connection((host, port), timeout=1.0):
            dependencies["temporal"] = {"status": "up"}
    except Exception as e:
        logger.error(f"Health check: Temporal connection failed at {temporal_addr}: {e}")
        dependencies["temporal"] = {"status": "down", "error": str(e), "checked_addr": temporal_addr}
