...
class ConnectorService:
    ...
    async def get_active_connector_by_type(
        self,
        tenant_id: str,
        connector_type: ConnectorType
    ) -> Optional[Any]:
        """
        Find and return the first active connector for a given type.
        Checks memory cache and then persistent storage.
        """
        # 1. Get all connectors of this type
        configs = [c for c in self.registry.get_all_configs() if c.type == connector_type]
        ...
        for config in configs:
            ...
            if credentials:
                try:
                    return self.registry.create_connector(
                        config.id,
                        tenant_id,
                        credentials
                    )
                except Exception as e:
                    logger.error(f"Failed to create connector {config.id}: {e}")
                    continue
