...
class WorkflowService:
    ...
    async def update_hitl_settings(self, tenant_id: str, workflow_id: str, enabled: bool) -> bool:
        workflow = self.db.query(Workflow).filter(Workflow.id == workflow_id, Workflow.tenant_id == tenant_id).first()
        if not workflow:
            return False
        workflow.hitl_enabled = enabled
        self.db.commit()
        return True

    async def list_proposals(self, status: Optional[str] = None) -> List[WorkflowProposal]:

        query = self.db.query(WorkflowProposal)
        if status:
            query = query.filter(WorkflowProposal.status == status)
        return query.all()

    async def handle_proposal(self, proposal_id: str, action: str, admin_id: str, notes: Optional[str] = None) -> bool:

        proposal = self.db.query(WorkflowProposal).filter(WorkflowProposal.id == proposal_id).first()
        if not proposal:
            return False
    ...
    async def list_approval_tasks(self, tenant_id: str, status: Optional[str] = "pending") -> List[Any]:
        from app.models.workflow import ApprovalTask
        query = self.db.query(ApprovalTask).filter(ApprovalTask.tenant_id == tenant_id)
        if status:
            query = query.filter(ApprovalTask.status == status)
        return query.all()

    async def handle_approval_task(self, task_id: str, action: str, admin_id: str, notes: Optional[str] = None) -> bool:
        from app.models.workflow import ApprovalTask
        ...
        task.judged_by = admin_id
        task.judged_at = datetime.utcnow()
        task.notes = notes
        
        self.db.commit()
        return True

    async def create_approval_task(self, tenant_id: str, data: Dict[str, Any]) -> Any:
        from app.models.workflow import ApprovalTask
        task = ApprovalTask(
            tenant_id=tenant_id,
