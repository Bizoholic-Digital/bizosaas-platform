...
class DirectoryService:
    ...
    async def search(
        ...
        # Paginate and transform results to match frontend expectations
        paginated_listings = listings[(page - 1) * limit : page * limit]
        
        transformed_listings = []
        for l in paginated_listings:
            biz_data = self._transform_listing_to_frontend(l)
            transformed_listings.append(biz_data)

        return {
            "businesses": transformed_listings,
    ...
    def _transform_listing_to_frontend(self, listing: DirectoryListing) -> Dict[str, Any]:
        """Transform DB model to match frontend Business interface"""
        # Extract images from google_data if available
        images = []
        if listing.google_data and "photos" in listing.google_data:
            api_key = os.getenv("GOOGLE_MAPS_API_KEY", "AIzaSyBZxfvuglTrcCIZZfSVDTltjBWTgEuRLto")
            for photo in listing.google_data["photos"][:5]: # Take up to 5 photos
                photo_ref = photo.get("photo_reference")
                if photo_ref:
                    images.append(f"https://maps.googleapis.com/maps/api/place/photo?maxwidth=800&photoreference={photo_ref}&key={api_key}")

        return {
            "id": str(listing.id),
            "name": listing.business_name,
            "slug": listing.business_slug,
            ...
            "location": {
                ...
                "zipCode": listing.postal_code or "",
                "country": listing.country or "",
                "coordinates": {
                    "lat": float(listing.google_data.get("geometry", {}).get("location", {}).get("lat", 0)) if listing.google_data else 0,
                    "lng": float(listing.google_data.get("geometry", {}).get("location", {}).get("lng", 0)) if listing.google_data else 0
                }
            },
            "rating": float(listing.google_rating or 0),
            "reviewCount": listing.google_reviews_count or 0,
            "images": images,
            "featured": listing.status == "featured",
            "verified": listing.verification_status == "verified",
            "verification_status": listing.verification_status,
            "status": "open", # Simplified for now
            "tags": listing.tags or [],
            "socialMedia": listing.social_media or {},
            "pricing": listing.pricing_info or {"range": "$$", "currency": "USD", "description": ""},
            "amenities": listing.amenities or [],
            "viewCount": 0, # Initial value
            "claimStatus": "claimed" if listing.claimed else "unclaimed",
