...
class AdminPrimeCopilot:
    ...
    async def get_available_tools(self) -> List[Dict[str, Any]]:
        """
        Get all tools available to the admin agent via MCP Gateway.
        """
        # Admin agent acts on behalf of the platform owner (usually the first admin user)
        # For now, we'll try to resolve the primary admin user ID or use a system context
        
        # 1. Find a valid admin context
        from app.models.user import User
        admin = self.db.query(User).filter(User.role == "super_admin").first()
        if not admin:
            return []
            
        from app.services.mcp_gateway import MCPGateway
        gateway = MCPGateway(self.db)
        return await gateway.list_tools(str(admin.id))

    async def execute_tool(self, tool_name: str, args: Dict[str, Any], mcp_slug: str = None) -> Any:
        """
        ...
        # If mcp_slug is not provided, we might need to find which MCP has this tool
        # For now, we assume mcp_slug is passed or we brute-force search (inefficient but works for v1)
        
        if not mcp_slug:
            tools = await gateway.list_tools(str(admin.id))
            target_tool = next((t for t in tools if t["name"] == tool_name), None)
            if not target_tool:
                raise ValueError(f"Tool '{tool_name}' not found")
            mcp_slug = target_tool["mcp_slug"]
            
        return await gateway.call_tool(str(admin.id), mcp_slug, tool_name, args)
