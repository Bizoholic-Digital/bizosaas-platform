...
class KnowledgeGraph:
    ...
    def add_edge(self, edge: ToolRelationship):
        ...
        existing = next((e for e in self.edges if e.source_tool == edge.source_tool and e.target_tool == edge.target_tool), None)
        if existing:
            # Update strength/evidence if new one is more significant
            existing.strength = max(existing.strength, edge.strength)
            existing.evidence_count = max(existing.evidence_count, edge.evidence_count)
            return
    ...
    def record_interaction(self, db: Session, source_tool_id: str, target_tool_id: str, success: bool = True):
        """
        Record an interaction between two tools to strengthen their relationship.
        This forms the basis of the continuous feedback loop (KAG).
        """
        ...
        # Find existing edge in DB
        db_edge = db.query(KagRelationship).filter(
            KagRelationship.source_tool == source_tool_id,
            KagRelationship.target_tool == target_tool_id
        ).first()
        
        if db_edge:
            # strengthen existing relationship
            if success:
                db_edge.evidence_count += 1
                # strength is 0-100 in DB
                db_edge.strength = min(100, db_edge.strength + 5)
                logger.debug(f"Strengthened KAG edge in DB: {source_tool_id} -> {target_tool_id} (New Strength: {db_edge.strength})")
            else:
                db_edge.strength = max(0, db_edge.strength - 10)
        else:
            # Create new emergent relationship if success
            if success:
                new_db_edge = KagRelationship(
                    source_tool=source_tool_id,
                    target_tool=target_tool_id,
                    relationship_type="emergent_workflow",
                    strength=10,
                    evidence_count=1
                )
                db.add(new_db_edge)
                logger.info(f"Discovered and persisted new KAG pattern: {source_tool_id} -> {target_tool_id}")
        
        try:
            db.commit()
        except Exception as e:
            db.rollback()
            logger.error(f"Failed to persist KAG interaction: {e}")

        # Update local graph for immediate use (optional if graph is rebuilt frequently)
        edge = next((e for e in self.edges if e.source_tool == source_tool_id and e.target_tool == target_tool_id), None)
        if edge:
            if success:
            ...
            self.add_edge(ToolRelationship(
                source_tool=source_tool_id,
                target_tool=target_tool_id,
                relationship_type="emergent_workflow",
                strength=0.1,
...
class KnowledgeGraphBuilder:
    ...
    async def build_graph(self) -> KnowledgeGraph:
        ...
        from app.models.mcp import McpRegistry, KagRelationship, UserMcpInstallation
        
        mcps = self.db.query(McpRegistry).all()
        
        # Create nodes for each MCP
        for mcp in mcps:
            # Find users using this MCP via UserMcpInstallation
            installations = self.db.query(UserMcpInstallation).filter(
                UserMcpInstallation.mcp_id == mcp.id,
                UserMcpInstallation.status == 'installed'
            ).all()
            
            user_ids = {inst.user_id for inst in installations}
            
            node = ToolNode(
                id=mcp.slug,
                name=mcp.name,
                category=mcp.category.slug if mcp.category else "general",
                capabilities=mcp.capabilities or [],
                connected_tenants=user_ids
            )
            self.graph.add_node(node)
        
        # Load persistent relationships from DB
        logger.info("Loading persistent KAG relationships...")
        db_relationships = self.db.query(KagRelationship).all()
        for rel in db_relationships:
            self.graph.add_edge(ToolRelationship(
                source_tool=rel.source_tool,
                target_tool=rel.target_tool,
                relationship_type=rel.relationship_type,
                strength=rel.strength / 100.0,
                evidence_count=rel.evidence_count
            ))

        # Infer additional relationships from co-usage patterns
