...
class PorkbunConnector(BaseConnector, DomainPort):
    @classmethod
    def get_config(cls) -> ConnectorConfig:
        return ConnectorConfig(
            ...
            description="Developer-friendly domain registration with Porkbun.",
            icon="piggy-bank",
            version="1.0.0",
            auth_schema={
                "api_key": {"type": "string", "label": "API Key", "sensitive": True},
                "secret_key": {"type": "string", "label": "Secret Key", "sensitive": True},
    ...
    async def register_domain(self, domain: str, years: int = 1, contact_info: Dict[str, Any] = {}) -> Dict[str, Any]:
        logger.info(f"Porkbun: Registering {domain}")
        return {"success": True, "domain": domain}

    async def get_domain_info(self, domain: str) -> Optional[DomainInfo]:
        return DomainInfo(domain=domain, registrar="porkbun", status="active")
