...
class GoogleTagManagerConnector(BaseConnector):
    ...
    async def sync_data(self, resource_type: str, params: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        Sync data from GTM (if API access is configured).
        Supported resource_types: 'tags', 'triggers', 'variables'
        """
        # For MVP, we might just return the container configuration
        return {
            "container_id": self.credentials.get("container_id"),
            "status": "active"
        }
    ...
    async def perform_action(self, action: str, payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if action == "get_snippet":
            container_id = self.credentials.get("container_id")
            return {
                "head_snippet": f"<!-- Google Tag Manager --><script>(function(w,d,s,l,i){{w[l]=w[l]||[];w[l].push({{'gtm.start':new Date().getTime(),event:'gtm.js'}});var f=d.getElementsByTagName(s)[0],j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';j.async=true;j.src='https://www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);}})(window,document,'script','dataLayer','{container_id}');</script><!-- End Google Tag Manager -->",
                "body_snippet": f"<!-- Google Tag Manager (noscript) --><noscript><iframe src=\"https://www.googletagmanager.com/ns.html?id={container_id}\" height=\"0\" width=\"0\" style=\"display:none;visibility:hidden\"></iframe></noscript><!-- End Google Tag Manager (noscript) -->"
            }
