...
class WordPressConnector(BaseConnector, CMSPort):
    ...
    def _get_api_url(self, path: str) -> str:
        ...
        # Bedrock alignment: Check if we need /wp/ prefix for REST API
        # but the credentials.url should ideally point to the HOME url.
        if "wp-json" in base_url:
             return f"{base_url}/wp/v2/{path.lstrip('/')}"
        
        # Bedrock often puts the API under /wp/wp-json/ if the core is in /wp/
        # but if WP_HOME is the root, it might still serve from root via index.php
        # Most reliable way for Bedrock with core in /wp:
        api_root = f"{base_url}/wp-json"
        
        # Test if the user provided the /wp/ path in the URL already
