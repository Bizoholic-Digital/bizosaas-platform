...
try:
    ...
    saleor_secrets = vault.get_saleor_config()
    if saleor_secrets:
        print(f"Loading {len(saleor_secrets)} secrets from Vault into environment...")
        for key, value in saleor_secrets.items():
            # Update os.environ so subsequent getenv calls find the secrets
            if key not in os.environ:
                os.environ[key] = str(value)
            # Also update if it IS in environ? Usually env vars override config, 
            # but here Vault is the source of truth. 
            # Let's prefer Vault if the env var is a default/placeholder.
            # For now, we'll stick to "Env vars override Vault" to allow local overrides,
            # unless the env var is empty.
            elif not os.environ[key]:
                 os.environ[key] = str(value)
except Exception as e:
    print(f"Vault integration warning: {e}")
# ===========================================================
...
MIDDLEWARE = [
    ...
    "saleor.multitenancy.middleware.OrganizationMiddleware",  # Multi-tenant organization resolution
    "django.middleware.common.CommonMiddleware",
    "saleor.core.middleware.jwt_refresh_token_middleware",
    "saleor.multitenancy.middleware.OrganizationUserMiddleware",  # Organization membership context
    "saleor.multitenancy.middleware.OrganizationDataIsolationMiddleware",  # Data isolation
]
...
INSTALLED_APPS = [
    ...
    "saleor.thumbnail",
    "saleor.schedulers",
    "saleor.multitenancy",  # Multi-tenant organization support
    # External apps
    "django_measurement",
    "mptt",
    "django_countries",
