...
export function PlatformBranding({ platform = 'BIZOSAAS', size = 'md', className }: PlatformBrandingProps) {
  const getTitle = () => {
    switch (platform) {
      case 'CLIENT_PORTAL':
  ...
  return (
    <div className={className}>
      <div className="flex items-center space-x-2">
        <div className={`${sizeClasses.icon} bg-blue-600 rounded-lg flex items-center justify-center`}>
        ...
        <div>
          ...
          <p className={`${sizeClasses.subtitle} text-gray-600 dark:text-gray-400`}>
            {platform === 'BIZOSAAS' ? 'Super Admin Dashboard' : 'Platform Management'}
          </p>
        </div>
      </div>
    </div>
...
// Export theme provider for compatibility
export function PlatformThemeProvider({ platform, children }: { platform: string; children: React.ReactNode }) {
