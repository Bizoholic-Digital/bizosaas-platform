...
BRAIN_API_URL = os.getenv("BRAIN_API_URL", "http://localhost:8001")  # FastAPI Brain AI Agentic Gateway
...
def require_role(required_roles: list[str]):
    ...
    async def role_dependency(user_session: UserSession = Depends(require_authentication)) -> UserSession:
...
def require_permission(required_permissions: list[str]):
    ...
    async def permission_dependency(user_session: UserSession = Depends(require_authentication)) -> UserSession:
        user_permissions = set(user_session.permissions)
...
async def require_admin_access(user_session: UserSession = Depends(require_authentication)) -> UserSession:
    """Require admin-level access (super_admin or tenant_admin)"""
...
async def require_super_admin(user_session: UserSession = Depends(require_authentication)) -> UserSession:
...
async def get_users(user_session: UserSession = Depends(require_admin_access)):
    """Get users list - Admin access only"""
    return {
        "message": f"Users list accessed by {user_session.email}",
        "access_level": user_session.role,
        "tenant_scope": user_session.tenant_id if user_session.role != "super_admin" else "global"
    }

@app.get("/api/admin/system/config")
async def get_system_config(user_session: UserSession = Depends(require_super_admin)):
    """Get system configuration - Super Admin only"""
    return {
        "message": f"System config accessed by {user_session.email}",
        "access_level": user_session.role,
        "config": {
            "multi_tenant": True,
            "platforms_enabled": ["bizoholic", "coreldove", "directory", "ai-chat"],
            "auth_service": UNIFIED_AUTH_URL,
            "session_timeout": 86400
        }
    }

@app.get("/api/admin/analytics")
async def get_admin_analytics(user_session: UserSession = Depends(require_admin_access)):
    """Get admin analytics dashboard data"""
    return {
        "message": f"Analytics accessed by {user_session.email}",
        "access_level": user_session.role,
        "analytics": {
            "total_users": 247 if user_session.role == "super_admin" else 42,
            "active_sessions": 18 if user_session.role == "super_admin" else 8,
            "platform_usage": {
                "bizoholic": 156,
                "coreldove": 98,
                "directory": 67,
                "ai-chat": 234
            } if user_session.role == "super_admin" else {
                "tenant_specific": "Limited view for tenant admin"
            }
        }
    }
...
async def get_ai_agents_status(
    ...
    """Get AI agents status - Admin access required"""
    return {
        "message": f"AI agents status accessed by {user_session.email}",
        "access_level": user_session.role,
        "agents": {
            "total_agents": 88,
            "active_agents": 84,
            "failed_agents": 2,
            "maintenance_agents": 2,
            "categories": {
                "marketing": 18,
                "ecommerce": 16, 
                "analytics": 14,
...
@app.post("/api/admin/ai-agents/{agent_id}/restart")
async def restart_ai_agent(
    agent_id: str,
    user_session: UserSession = Depends(require_permission(["agent.restart", "admin.*"]))
):
    """Restart AI agent - Requires specific permission"""
    return {
        "message": f"Agent {agent_id} restart initiated by {user_session.email}",
        "agent_id": agent_id,
        "status": "restarting",
        "permissions": user_session.permissions
    }
...
# ===============================================
# BRAIN API ROUTES - FastAPI AI Agentic Gateway
# ===============================================

@app.api_route("/api/brain/{full_path:path}", methods=["GET", "POST", "PUT", "DELETE", "PATCH"])
async def brain_api_proxy(request: Request, full_path: str):
    """
    Proxy all Brain API calls to the FastAPI AI Agentic Gateway
    Routes all /api/brain/* requests to the centralized gateway
    """
    method = request.method
    url = f"{BRAIN_API_URL}/api/brain/{full_path}"
    ...
    # Prepare headers for gateway
    headers = {
        "Content-Type": "application/json",
        "Host": "localhost:3000",  # Set tenant context
        "X-Tenant": "tailadmin-v2",
        "X-Platform-Type": "admin-dashboard"
    }
    
    # Add any authentication headers from the original request
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.request(
                ...
                logger.info(f"[BRAIN API] {method} /api/brain/{full_path} -> {response.status}")
        ...
        logger.error(f"[BRAIN API] Error proxying to gateway: {e}")
        
        # Return fallback response for development
        fallback_data = {
            "service": full_path.split('/')[0] if '/' in full_path else "unknown",
            "endpoint": full_path,
            "method": method,
            "status": "fallback",
            "message": f"TailAdmin v2 Brain API proxy - Gateway unavailable",
            "timestamp": datetime.utcnow().isoformat(),
            "source": "tailadmin-v2-fallback"
        }
