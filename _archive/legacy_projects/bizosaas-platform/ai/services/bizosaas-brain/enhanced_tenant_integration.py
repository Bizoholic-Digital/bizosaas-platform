...
from shared.rls_manager import RLSManager, create_rls_manager
from shared.rls_middleware import RLSMiddleware
...
from realtime_collaboration_endpoints import setup_collaboration_endpoints
from collaborative_document_system import (
    CollaborativeDocumentManager,
    initialize_document_manager,
    get_document_manager
)

# Import security and compliance components
...
from security.security_middleware import setup_comprehensive_security
from security.security_api_endpoints import setup_security_endpoints
...
async def initialize_tenant_system(app: FastAPI):
    ...
    global rls_manager, ai_coordinator, collaboration_service, document_manager
    global threat_detection_engine, compliance_manager, security_audit_logger

    logger.info("Initializing enhanced tenant context system with comprehensive security")

    # Initialize RLS manager
    if not rls_manager:
        rls_manager = await create_rls_manager()

    # Initialize AI coordinator
    if not ai_coordinator:
        ai_coordinator = await initialize_ai_coordinator(rls_manager)

    # Initialize collaboration service
    if not collaboration_service:
        collaboration_service = initialize_collaboration_service(ai_coordinator, rls_manager)

    # Initialize document manager
    if not document_manager:
        document_manager = initialize_document_manager(rls_manager)
    ...
    if not all([threat_detection_engine, compliance_manager, security_audit_logger]):
        ...
        ) = initialize_security_framework(rls_manager)
    ...
    setup_comprehensive_security(app, {
        ...
        "auto_block_suspicious_ips": False  # Set to True in production
    })
    ...
    # Setup collaboration endpoints
    setup_collaboration_endpoints(app)

    # Setup security and compliance endpoints
    setup_security_endpoints(app)

    # Initialize tenant context manager
    # In production, this would load configuration from database
    logger.info("Enhanced tenant context system with real-time collaboration and comprehensive security initialized successfully")
