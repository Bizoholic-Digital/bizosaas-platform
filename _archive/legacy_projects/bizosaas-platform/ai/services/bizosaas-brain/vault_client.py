...
class VaultClient:
    ...
    def __init__(self, vault_addr: str = None, vault_token: str = None):
        ...
        self.mount_path = os.getenv('VAULT_MOUNT_PATH', 'bizosaas')
        self.client = None
    ...
    def _initialize_client(self):
        ...
        try:
            ...
            # Verify authentication (but don't fail if connection is slow)
            try:
                if self.client.is_authenticated():
                    logger.info(f"Successfully connected to Vault at {self.vault_addr}")
                else:
                    logger.warning(f"Vault client created but not yet authenticated at {self.vault_addr}")
            except Exception as auth_e:
                logger.warning(f"Could not verify Vault authentication immediately: {auth_e}")
                # Don't fail - allow lazy authentication

        except Exception as e:
            logger.error(f"Failed to create Vault client: {e}")
            # Create a dummy client anyway to avoid breaking the application
            self.client = None
