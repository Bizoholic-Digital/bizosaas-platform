...
export function AdvancedSearchBar({
  ...
  // Auto-detect location on mount if not provided
  useEffect(() => {
    if (!filters.location && typeof navigator !== 'undefined' && navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        async (position) => {
          try {
            const { latitude, longitude } = position.coords;
            // Reverse geocode to get a readable string if possible, or just set coordinates
            // For now, let's just use "My Location" or set coordinates in filters
            setFilters(prev => ({
              ...prev,
              latitude,
              longitude,
  ...
  // Removed: Auto-trigger onChange when debounced values change to prevent excessive refreshes
  // Submitting the form or clicking a suggestion will now be the main trigger
