...
import { ApiResponse } from '@/lib/api/brain-client'
...
export default function WorkflowManagementPage() {
  ...
  const handleToggleHitl = async (workflowId: string, enabled: boolean) => {
    try {
      await adminApi.updateHitlSettings(workflowId, enabled);
      toast.success(`HITL ${enabled ? 'enabled' : 'disabled'} for workflow`);
      loadWorkflowData();
    } catch (error) {
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <Tabs defaultValue="overview" className="space-y-4">
        <TabsList className="grid w-full grid-cols-6">
          ...
          <TabsTrigger value="hitl">HITL Governance</TabsTrigger>
      ...
      {/* HITL Governance Tab Content (Integrated below in return) */}
    </div>
  );
}

// Helper components for HITL Governance
function HitlGovernanceTab({
  workflows,
  onToggleHitl
}: {
  workflows: WorkflowStatus[],
  onToggleHitl: (id: string, enabled: boolean) => void
}) {
  const [proposals, setProposals] = useState<any[]>([]);

  useEffect(() => {
    adminApi.getProposals().then((res: ApiResponse<any[]>) => setProposals(res.data || []));
  }, []);

  const handleAction = async (id: string, action: 'approve' | 'reject') => {
    try {
      await adminApi.handleProposal(id, action);
    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Control which agentic workflows require manual approval before execution.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            {workflows.filter(wf => wf.type === 'ai-agent').map(wf => (
              <div key={wf.id} className="flex items-center justify-between p-4 border rounded-xl">
                <div>
                  <h4 className="font-bold">{wf.name}</h4>
                  <p className="text-sm text-gray-500">Global HITL status for all tenants</p>
                </div>
                <div className="flex items-center gap-4">
                  <Badge variant={(wf as any).hitlEnabled ? "default" : "secondary"}>
                    {(wf as any).hitlEnabled ? "HITL Enabled" : "Autonomous"}
                  </Badge>
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => onToggleHitl(wf.id, !(wf as any).hitlEnabled)}
                  >
                    {(wf as any).hitlEnabled ? "Disable HITL" : "Enable HITL"}
                  </Button>
                </div>
              </div>
            ))}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Workflows discovered or proposed by AI agents that require administrative review.
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            {proposals.length === 0 ? (
              <div className="flex flex-col items-center py-10 text-gray-400">
                <CheckCircle className="h-10 w-10 opacity-20 mb-2" />
                <p>No pending proposals from agents.</p>
              </div>
            ) : (
              proposals.map(proposal => (
                <div key={proposal.id} className="p-6 border rounded-2xl bg-gray-50/50 space-y-4">
                  <div className="flex justify-between items-start">
                    <div>
                      <h4 className="text-lg font-black">{proposal.name}</h4>
                      <p className="text-gray-600">{proposal.description}</p>
                      <Badge className="mt-2">{proposal.type}</Badge>
                    </div>
                    <div className="flex gap-2">
                      <Button variant="outline" size="sm" onClick={() => handleAction(proposal.id, 'reject')} className="text-red-600">Reject</Button>
                      <Button size="sm" onClick={() => handleAction(proposal.id, 'approve')} className="bg-indigo-600">Approve & Deploy</Button>
                    </div>
                  </div>
                  {proposal.impactAnalysis && (
                    <div className="p-3 bg-indigo-50/30 rounded-xl border border-indigo-100 text-sm">
                      <p className="font-bold text-indigo-900 mb-1">Impact Analysis</p>
                      <p className="text-indigo-700">{proposal.impactAnalysis}</p>
                    </div>
                  )}
                </div>
              ))
