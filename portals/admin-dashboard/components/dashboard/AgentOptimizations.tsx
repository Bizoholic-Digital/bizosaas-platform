...
export function AgentOptimizations() {
    ...
    const fetchOptimizations = async () => {
        try {
            const res = await fetch('/api/brain/agents/optimizations')
            if (res.ok) {
    ...
    const handleApprove = async (id: string) => {
        try {
            const res = await fetch(`/api/brain/agents/optimizations/${id}/approve`, { method: 'POST' })
            if (res.ok) {
    ...
    const handleToggleAuto = async (id: string, enabled: boolean) => {
        try {
            const res = await fetch(`/api/brain/agents/optimizations/${id}/toggle-auto`, {
                method: 'POST',
                                    ...
                                    <div className="flex flex-row md:flex-col items-center md:items-end gap-4 shrink-0">
                                        <div className="flex items-center gap-3 p-3 bg-slate-100 dark:bg-slate-800 rounded-2xl border border-slate-200 dark:border-slate-700">
                                            <div className="flex flex-col">
                                                <span className="text-[10px] font-black uppercase text-slate-500">Auto-Execute</span>
                                                <span className="text-[9px] text-slate-400">AI controlled</span>
                                            </div>
                                            <Switch
                                        ...
                                        {opt.status === 'pending' ? (
                                            <Button
                                                ...
                                                className="bg-indigo-600 hover:bg-indigo-700 text-white font-black rounded-xl px-6 shadow-lg shadow-indigo-500/20 w-full md:w-auto"
                                            >
                                                Approve & Run
                                            </Button>
                                        ) : (
                                            <div className="flex items-center gap-2 text-green-500 font-black text-sm px-4 py-2 bg-green-50 dark:bg-green-900/20 rounded-xl">
                ...
                {optimizations.length > 0 && (
                    <div className="p-4 bg-slate-50 dark:bg-slate-900/80 border-t border-slate-100 dark:border-slate-800 flex justify-between items-center">
                        <div className="flex items-center gap-2 text-[10px] font-black uppercase text-slate-400">
                            ...
                            Potential Cost Savings: <span className="text-green-500">$12.40 / mo</span>
                        </div>
