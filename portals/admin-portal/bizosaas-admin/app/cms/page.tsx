...
export default function CMSManagementPage() {
  ...
  // Fetch CMS data from FastAPI Central Hub
  useEffect(() => {
    fetchCMSData()
  }, [])
  ...
  const fetchCMSData = async () => {
    setLoading(true)
    try {
      const [pagesRes, mediaRes, submissionsRes] = await Promise.allSettled([
        fetch(`${BRAIN_API_URL}/api/brain/wagtail/pages`, {
          headers: { 'Content-Type': 'application/json' }
        }),
        fetch(`${BRAIN_API_URL}/api/brain/wagtail/media`, {
          headers: { 'Content-Type': 'application/json' }
        }),
        fetch(`${BRAIN_API_URL}/api/brain/wagtail/submissions`, {
          headers: { 'Content-Type': 'application/json' }
        })
      ])
      ...
      if (pagesRes.status === 'fulfilled' && pagesRes.value.ok) {
        ...
        setPages(pagesData.pages || fallbackPages)
      } else {
      ...
      if (mediaRes.status === 'fulfilled' && mediaRes.value.ok) {
        ...
        setMedia(mediaData.media || fallbackMedia)
      } else {
      ...
      if (submissionsRes.status === 'fulfilled' && submissionsRes.value.ok) {
        ...
        setSubmissions(submissionsData.submissions || fallbackSubmissions)
      } else {
