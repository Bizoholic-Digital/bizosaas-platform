...
export function OnboardingWizard() {
    ...
    const handleLaunch = async () => {
        ...
        try {
            ...
            if (res.ok) {
                toast({
                    title: "Ignition Successful",
                    description: "Your platform is being provisioned. Redirecting...",
                });
                setTimeout(() => {
            ...
            toast({
                title: "Launch Failed",
                description: "There was an error during ignition. Please try again.",
                variant: "destructive"
            });
        } finally {
                ...
                <Card className="w-full shadow-2xl bg-card/80 backdrop-blur-xl border-border/50 overflow-hidden min-h-[600px] md:min-h-[650px] flex flex-col rounded-none md:rounded-3xl border-0 md:border">
                    <CardContent className="p-0 flex-1 flex flex-col">
                        ...
                        <div className="border-t bg-muted/10 p-6 md:p-10 flex flex-col sm:flex-row justify-between items-center gap-6">
                            ...
                            {isLastStep ? (
                                <Button onClick={handleLaunch} disabled={isSubmitting} className="bg-green-600 hover:bg-green-700 text-white px-8 py-6 text-lg shadow-lg font-black uppercase tracking-tight">
                                    {isSubmitting ? 'Igniting...' : 'Ignite & Launch 🚀'}
                                </Button>
                            ) : (
                                <Button
                                    onClick={() => {
