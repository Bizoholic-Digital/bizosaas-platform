...
export function CategorizedToolSelectionStep({ data, onUpdate, state }: Props) {
    ...
    const CORE_SLUGS = ['wordpress', 'shopify', 'fluentcrm', 'hubspot', 'google-tag-manager', 'google-analytics-4', 'woocommerce'];
    ...
    useEffect(() => {
        ...
        if (state.digitalPresence.cmsType === 'wordpress') {
            if (!autoSelected.has('wordpress')) { autoSelected.add('wordpress'); hasChanges = true; }
            if (!autoSelected.has('google-search-console')) { autoSelected.add('google-search-console'); hasChanges = true; }
        }

        if (state.digitalPresence.cmsType === 'shopify') {
            if (!autoSelected.has('shopify')) { autoSelected.add('shopify'); hasChanges = true; }
        }

        // 2. CRM Foundations
        if (state.digitalPresence.crmType === 'fluentcrm') {
            if (!autoSelected.has('fluentcrm')) { autoSelected.add('fluentcrm'); hasChanges = true; }
        }
        if (state.digitalPresence.crmType === 'hubspot') {
            if (!autoSelected.has('hubspot')) { autoSelected.add('hubspot'); hasChanges = true; }
        }

        // 3. E-commerce Foundations
        if (state.digitalPresence.ecommerceType === 'woocommerce') {
            if (!autoSelected.has('woocommerce')) { autoSelected.add('woocommerce'); hasChanges = true; }
        }

        // 4. Analytics Auto-selection
        const analytics = [
            ...(state?.analytics.auditedServices?.essential || []),
            ...(state?.analytics.auditedServices?.optional || [])
        ].map(s => s.service?.toUpperCase());

        if (analytics.includes('GA4') || analytics.includes('GOOGLE ANALYTICS')) {
            if (!autoSelected.has('google-analytics-4')) { autoSelected.add('google-analytics-4'); hasChanges = true; }
        }
        if (analytics.includes('GTM') || analytics.includes('GOOGLE TAG MANAGER')) {
            if (!autoSelected.has('google-tag-manager')) { autoSelected.add('google-tag-manager'); hasChanges = true; }
        }

        // 5. Default Project Management
        if (!autoSelected.has('plane')) {
            autoSelected.add('plane');
            hasChanges = true;
        }

        // Only update if we have new auto-selections to prevent infinite loops
        if (hasChanges) {
            onUpdate({ ...data, selectedMcps: Array.from(autoSelected) });
        }
    }, [state?.digitalPresence, state?.analytics, data.selectedMcps]);
