...
export const EcommerceContent: React.FC<EcommerceContentProps> = ({ activeTab }) => {
    ...
    const refreshData = async () => {
        const endpoints = {
            ...
            customers: '/api/brain/saleor/customers',
            inventory: '/api/brain/saleor/products',
            coupons: '/api/brain/saleor/products',
            reviews: '/api/brain/saleor/products'
        };
        ...
        try {
            const responses = await Promise.all(
                Object.entries(endpoints).map(async ([key, endpoint]) => {
                    try {
                        ...
                        let items = [];
                        if (key === 'products') items = data.products || [];
                        else if (key === 'orders') items = data.orders || [];
                        else if (key === 'customers') items = data.customers || [];
                        else if (key === 'inventory') items = data.products || [];
                        else if (key === 'coupons') items = data.coupons || [];
                        else if (key === 'reviews') items = data.reviews || [];
            ...
            setEcomData(Object.fromEntries(responses) as any);
        } catch (error) {
    ...
    const handleCreate = async (type: string, data: any) => {
        try {
            const endpoint = `/api/brain/saleor/${type}`;
            const response = await fetch(endpoint, {
                method: 'POST',
    ...
    const handleUpdate = async (type: string, id: string, data: any) => {
        try {
            let url = `/api/brain/saleor/${type}`;
            if (type === 'products') url += `?product_id=${id}`;
            else if (type === 'orders') url += `?order_id=${id}`;
            else if (type === 'customers') url += `?customer_id=${id}`;
        ...
        const fetchEcomData = async () => {
            try {
                const endpoints = {
                    ...
                    orders: '/api/brain/saleor/orders',
                    customers: '/api/brain/saleor/customers',
                    inventory: '/api/brain/saleor/products', // Reuse products for inventory
                    coupons: '/api/brain/saleor/products', // Placeholder
                    reviews: '/api/brain/saleor/products' // Placeholder
                };
                ...
                console.error('E-commerce data fetch error:', error);
            }
        };
