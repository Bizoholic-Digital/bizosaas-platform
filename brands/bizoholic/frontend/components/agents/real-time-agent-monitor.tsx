              ...
              <ScrollArea className="h-[600px]">
                <div className="space-y-4">
                  {filteredAgents.map((agent) => (
                    <div
                      key={agent.id}
                      className={`p-4 border rounded-lg cursor-pointer transition-colors ${selectedAgent?.id === agent.id ? 'bg-accent' : 'hover:bg-muted'
                        }`}
                    ...
                    >
                      ...
                      {agent.status === 'running' && user?.role === 'super_admin' && (
