...
export function LanguageSelector({ onLanguageChange, className = "" }: LanguageSelectorProps) {
  ...
  const [multilingualManager] = useState(() =>
    new MultilingualManager(process.env.NEXT_PUBLIC_API_BASE_URL || 'https://api.bizoholic.net', user?.tenant_id || 'demo')
  )
  ...
  const initializeMultilingual = async () => {
    setIsLoading(true)
    try {
      await multilingualManager.initialize(user?.tenant_id || 'demo', user?.id || 'demo-user', user?.role || 'user')
