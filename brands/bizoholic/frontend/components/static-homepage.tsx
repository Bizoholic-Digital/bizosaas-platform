          ...
          <div className="flex overflow-x-auto pb-6 -mx-4 px-4 snap-x snap-mandatory md:grid md:gap-8 md:grid-cols-2 lg:grid-cols-3 md:overflow-visible md:pb-0 md:mx-0 md:px-0 hide-scrollbar">
            {loading ? (
              ...
              Array.from({ length: 6 }).map((_, index) => (
                <Card key={`skeleton-${index}`} className="relative min-w-[280px] w-[85vw] md:w-auto md:min-w-0 snap-center">
                  <CardHeader>
              ...
              services.map((service, index) => {
                const ServiceCard = service.link ?
                  ({ children }: { children: React.ReactNode }) => (
                    <Link href={service.link} className="block min-w-[280px] w-[85vw] md:w-auto md:min-w-0 snap-center h-full">
                      <Card className="relative hover:shadow-lg transition-shadow cursor-pointer group h-full">
                        {children}
                      </Card>
                    </Link>
                  ) :
                  ({ children }: { children: React.ReactNode }) => (
                    <div className="min-w-[280px] w-[85vw] md:w-auto md:min-w-0 snap-center h-full">
                ...
                return (
                  <ServiceCard key={index}>
                    ...
                    <CardContent>
                      ...
                      {service.link && (
                        <div className="flex items-center text-primary text-sm font-medium group-hover:gap-2 transition-all mt-auto">
