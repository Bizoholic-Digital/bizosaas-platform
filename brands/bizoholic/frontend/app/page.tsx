...
export default function HomePage() {
  ...
  const fetchHomepageContent = async () => {
    try {
      ...
      if (homepageRes.ok) {
        homepageData = await homepageRes.json()
      }
      ...
      if (servicesRes.ok) {
        ...
        setServicePages(featuredServices)

        // If homepage has no features, convert services to features format
        if (homepageData?.homepage && (!homepageData.homepage.features || homepageData.homepage.features.length === 0)) {
          homepageData.homepage.features = featuredServices.map((service: ServicePage) => ({
            icon: service.icon,
            title: service.title,
  ...
  const content = homepageContent ? {
    ...homepageContent,
    hero_cta_url: "http://localhost:3003/register" // Force correct URL for Client Portal
  } : fallbackContent
  return (
