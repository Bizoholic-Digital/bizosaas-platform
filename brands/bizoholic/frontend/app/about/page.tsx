...
export default function AboutPage() {
  ...
  useEffect(() => {
    const fetchAboutContent = async () => {
      try {
        const response = await fetch('/api/brain/wordpress/pages?slug=about')
        if (response.ok) {
          const data = await response.json()
          if (Array.isArray(data) && data.length > 0) {
            const page = data[0]
            if (page.acf) {
              // Map ACF fields if available
              const mappedContent = {
                company_name: 'Bizoholic', // Keep hardcoded or from settings
                hero_subtitle: page.acf.hero_subtitle || page.excerpt?.rendered?.replace(/<[^>]*>?/gm, '') || '',
                team_members: page.acf.team_members?.map((m: any) => ({
                  name: m.name,
                  role: m.role,
                  image: m.image?.url || "https://placehold.co/400x400/2563eb/ffffff?text=User",
                  bio: m.bio
                })),
                stats: page.acf.stats?.map((s: any) => ({
                  number: s.number,
                  label: s.label
                }))
              }
        ...
        <div className="container">
          <div className="text-center">
            <h1 className="text-4xl md:text-6xl font-bold text-foreground mb-6">
              About <span className="text-primary">{content?.company_name || 'Bizoholic'}</span>
            </h1>
            <p className="text-xl text-muted-foreground max-w-3xl mx-auto mb-8">
              {content?.hero_subtitle || "We're a team of marketing mavericks, data scientists, and creative visionaries who believe every business deserves to be addicted to success."}
            </p>
