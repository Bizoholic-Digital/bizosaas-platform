...
export function PrimaryAIInterface({ onTabNavigate }: { onTabNavigate?: (tabId: string) => void }) {
  ...
  const [context, setContext] = useState<ConversationContext>({
    tenant_id: user?.user.tenant_id || 'demo',
  ...
  useEffect(() => {
    ...
    const welcomeMessage: ChatMessage = {
      id: 'welcome',
      type: 'ai',
      content: `Hello ${user?.user.first_name || 'there'}! 👋 I'm your BizOSaaS AI assistant. 
...
`
  ...
  const handleDocumentProcessed = async (document: any) => {
    const notification: ChatMessage = {
      ...
      type: 'system',
      content: `📄 Document "${document.filename}" has been processed successfully${
        document.metadata?.ecommerce_potential 
      ...
      >
        <div className={`flex max-w-[85%] ${isUser ? 'flex-row-reverse' : 'flex-row'} items-start gap-3`}>
          <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${
            isUser ? 'bg-blue-500' : isSystem ? 'bg-yellow-500' : 'bg-green-500'
          }`}>
            {isUser ? (
              ...
              {!isUser && !isSystem && message.type === 'ai' && analyticsManager.current && (
                <FeedbackWidget
                  ...
                  onSubmitFeedback={async (feedback) => {
                    if (analyticsManager.current) {
                      return await analyticsManager.current.submitFeedback(feedback)
                    }
                    return false
                  }}
                  className="ml-2"
