...
# Compose IDs
BACKEND_COMPOSE_ID="2hd_XXqQX_30pZFMOMb7k" # CMS
FRONTEND_COMPOSE_ID="sFHT3Oiep0z9UZmvby1QM" # Thrillring

# Compose file paths (relative to GitHub repo root)
BACKEND_COMPOSE_PATH="./docker-compose.bizoholic-cms.yml"
FRONTEND_COMPOSE_PATH="./docker-compose.thrillring.yml"
...
EOF
    ...
    if echo "$response" | jq -e '.deploymentId' > /dev/null 2>&1; then
        ...
        log_success "$name deployment triggered (Queued)"
        return 0
    else
