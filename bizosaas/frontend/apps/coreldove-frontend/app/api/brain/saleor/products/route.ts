...
export async function GET(request: NextRequest) {
  try {
    ...
    const marketplace = searchParams.get('marketplace') || 'IN'
    ...
    if (first) queryParams.set('first', first)
    queryParams.set('marketplace', marketplace)
    queryParams.set('currency', currency)
    queryParams.set('limit', limit)
    
    // Debug: Log the constructed URL
    const brainApiUrl = `${BRAIN_API_URL}/api/brain/saleor/products?${queryParams.toString()}`
    console.log('🔍 Calling Brain API:', brainApiUrl)
    
    // Forward the request to the Brain API
    const response = await fetch(brainApiUrl, {
      headers: {
        'Content-Type': 'application/json',
        'Host': request.headers.get('host') || 'localhost:3002', // Set proper host for tenant resolution
      },
